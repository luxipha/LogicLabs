#!/usr/bin/env node
/**
 * Transcribes the waterwheel voiceover with Whisper.cpp to get word-level
...
const INPUT = path.join(ROOT, 'src/assets/waterwheel/waterwheel (1).mp3');
const WAV = path.join(ROOT, '.scratch/waterwheel-narration-16k.wav');
const OUT = path.join(ROOT, '.scratch/waterwheel-transcript.json');
...
for (const cap of captions) {
  ...
  if (cap.endMs - lastEnd > 600 || sentence.length > 80) {
    console.log(`[${(cap.startMs / 1000).toFixed(2)}s] ${sentence.trim()}`);
    sentence = '';
  }
  lastEnd = cap.endMs;
}
if (sentence.trim()) {
  console.log(`[end ${(lastEnd / 1000).toFixed(2)}s] ${sentence.trim()}`);
}
